...
export const AIPanel = ({ 
  ...
  return (
    <div className="flex flex-col h-full bg-[#1e1e1e] text-gray-200 select-text overflow-hidden">
                      ...
                      if (!inline && match) {
                        return (
                          <div className="my-2 rounded bg-[#1e1e1e] border border-white/10 overflow-hidden not-prose">
                            ...
                            <pre className="p-3 text-[11.5px] font-mono overflow-x-auto text-gray-200 leading-normal scrollbar-thin">
                              <code>{children}</code>
                            </pre>
                          </div>
                        );
      ...
      {/* Collapsible Quick Action Chips */}
      <div className="bg-[#252526] border-t border-white/10 shrink-0">
        <button
          onClick={() => setShowActionChips(prev => !prev)}
          className="w-full px-3 py-1.5 flex items-center justify-between text-left hover:bg-white/5 transition-colors cursor-pointer group"
          title={showActionChips ? "Collapse agent actions" : "Expand agent actions"}
        >
          <div className="flex items-center space-x-1.5 text-[10px] uppercase font-bold text-gray-400 group-hover:text-gray-200 tracking-wider">
            {showActionChips ? (
              <ChevronDown className="w-3 h-3 text-indigo-400 transition-transform" />
            ) : (
              <ChevronRight className="w-3 h-3 text-indigo-400 transition-transform" />
            )}
            <span>Autonomous Agent Actions</span>
          </div>
          <span className="text-[9px] text-indigo-300 font-mono">
            {showActionChips ? '▲ collapse' : '▼ expand'}
          </span>
        </button>
        ...
        {showActionChips && (
          <div className="px-3 pb-2.5 pt-0.5 flex flex-wrap gap-1.5">
            <button
              ...
              className="flex items-center space-x-1 px-2.5 py-1 bg-white/5 hover:bg-white/10 text-gray-300 hover:text-white rounded text-[11px] transition-all disabled:opacity-40 disabled:pointer-events-none border border-white/5 shadow-sm"
            >
      ...
      <div className="p-3 bg-[#1e1e1e] border-t border-white/10 shrink-0 relative">
        ...
        {showMentionMenu && filteredMentionFiles.length > 0 && (
          <div className="absolute bottom-full left-3 right-3 mb-2 bg-[#252526] border border-indigo-500/40 rounded-lg shadow-2xl overflow-hidden z-50 max-h-48 flex flex-col">
            ...
            <div className="overflow-y-auto max-h-48 scrollbar-thin scrollbar-thumb-white/10 hover:scrollbar-thumb-white/20 scrollbar-track-transparent divide-y divide-white/5">
